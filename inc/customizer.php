...
/*
<div id="welcome-box">
				<h2><?php echo get_theme_mod( 'oursavior_featuredHeadtext' );  ?>
	</h2>
	<?php echo get_theme_mod( 'oursavior_featuredTA' );  ?>
		</div>
		*/ /** * Binds JS handlers to make Theme Customizer preview reload changes asynchronously. */ function dm_com_customize_preview_js() { wp_enqueue_script( 'dm_com_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20130508', true ); } add_action( 'customize_preview_init', 'dm_com_customize_preview_js' ); ///** // * Adds a new section to use custom controls in the WordPress customiser // * // * @param Obj $wp_manager - WP Manager // * // * @return Void // */ //private function custom_sections( $wp_manager ) //{ // // Add a text editor control // require_once dirname(__FILE__) . '../text-editor-custom-control.php'; // $wp_manager->add_setting( 'text_editor_setting', array( // 'default' => '', // ) ); // $wp_manager->add_control( new Text_Editor_Custom_Control( $wp_manager, 'text_editor_setting', array( // 'label' => 'Text Editor Setting', // 'section' => 'customiser_demo_custom_section', // 'settings' => 'text_editor_setting', // 'priority' => 11 // ) ) ); // } //}
